  ...
  if(colonne == 0 && etat == 3) {
    ...
    if (nfc.tagPresent(50))
    {
      ...
      if(tag.hasNdefMessage()){
        ...
        // PrintHexChar(payload,longueur);
        // std::string str(payload);
        String value = String(sizeof(payload));
        ...
        Serial.println("");
        Serial.println(String(char(payload[3]))+String(char(payload[4])));
        if(zapAuth)
        {
          requestServer("zap/"+String(char(payload[3]))+String(char(payload[4])));
          zapAuth = false;
        }
        delete payload;
      }
